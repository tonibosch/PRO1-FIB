...
string containtment(Point p,Rectangle r) {
  string s;
  if(p.x < r.xmin or p.x > r.xmax or p.y < r.ymin or p.y > r.ymax) s = "outside";
  else if (p.x == r.xmin or p.x == r.xmax or p.y == r.ymin or p.y == r.ymax) s = "border";
  else s = "inside";
